...
| P1 | Cuba inbound flights map | `src/services/cuba-flights.ts` | `/api/flightaware-cuba` | `api/flightaware-cuba.js` | Requires user-provided API key | ✅ FlightAware data is cached for cost control; curated Cuba routes remain as fallback. |
| P2 | Summaries | `src/services/summarization.ts` | `/api/ai` | `api/ai.js` | Requires user-provided API key | ✅ Explicit fallback chain: Hugging Face → Alibaba Qwen → browser model. |
